...
class Registration {
    ...
    async registerUser(reqBody) {
        ...
        if (await this.checkpasswordLength(password)) {
            return `<div class="text-red-500">Password cannot exceed 16 characters</div>`;
        }
        ...
        let connection;
        try {
            connection = await getConnectionWithRetry(pool);
...
async function getConnectionWithRetry(pool, retryInterval = 30000, maxRetries = 10) {
    for (let i = 0; i < maxRetries; i++) {
        try {
            return await pool.getConnection();
        } catch (error) {
            console.error(`Attempt ${i + 1}: Unable to connect to the database. Retrying in ${retryInterval / 1000} seconds.`);
